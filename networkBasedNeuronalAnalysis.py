...
all_neurons_filtered = evaluate_neuron_significance_based_on_fixation(allNeurons, saccad_df, 300)
saccad_df_filtered = saccade_df(all_neurons_filtered, 3000)
logging.info("Total number of " + "%s neuron" % len(all_neurons_filtered) +
             " out of " + "%s neuron" % len(allNeurons) + " selected as significant.")

allNeurons = all_neurons_filtered
...
if spiking_data == 'Raw':
    ...
    firingRate = generate_lagged_all_epoch_dict(allNeurons, saccad_df, 0, 'evokedFr',
                                                'pd.DataFrame', epoch_list)
    spikeCounts = generate_lagged_all_epoch_dict(allNeurons, saccad_df, 0, 'evoked',
                                                 'pd.DataFrame', epoch_list)
    neuronalData = generate_lagged_all_epoch_dict(allNeurons, saccad_df, 0, 'evoked',
    ...
    firingRate = generate_lagged_all_epoch_dict(allNeuronsVLMC, saccad_df_VLMC, 0, 'evokedFr',
                                                'pd.DataFrame', epoch_list)
    spikeCounts = generate_lagged_all_epoch_dict(allNeuronsVLMC, saccad_df_VLMC, 0, 'evoked',
                                                 'pd.DataFrame', epoch_list)
    neuronalData = generate_lagged_all_epoch_dict(allNeuronsVLMC, saccad_df_VLMC, 0, 'evoked',
                                                  'numpyArray', epoch_list)
    ...
    # get neural data information
    start_time = time.time()
    firing_rate_writer(allNeuronsVLMC, args)
    split_epoch_condition(firingRate, spikeCounts, args)
    logging.info('Raw data ingestion completed in:' + " %s minutes " % round((time.time() - start_time) / 60, 4))

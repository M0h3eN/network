...
# export JAVA_HOME=
export MASTER=local[*]                 		# Spark master url. eg. spark://master_addr:7077. Leave empty if you want to use local mode.
...
##
export SPARK_HOME='/opt/spark-2.4.4-bin-hadoop2.7'                             # (required) When it is defined, load it instead of Zeppelin embedded Spark libraries
export SPARK_SUBMIT_OPTIONS='--master local[*] --executor-memory 8G --driver-memory 4G'                   # (optional) extra options to pass to spark submit. eg) "--driver-memory 512M --executor-memory 1G".
